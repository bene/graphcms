...
func MaskToMultipleField(object *graphql.Object) (graphql.Field, error) {
	...
	return graphql.Field{
		...
		Resolve: func(p graphql.ResolveParams) (i interface{}, err error) {
			...
			tableName := p.Info.FieldName[:len(p.Info.FieldName)-1]
			...
			for _, s := range selections {
				if !s.HasSubSelect {
					...
					subSelections := getSelectedFields([]string{p.Info.FieldName, s.Name}, p)
